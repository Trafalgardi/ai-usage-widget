...
**Альтернатива через CLI:** OpenCode предоставляет команду `opencode stats`, которая
показывает статистику использования токенов и стоимость. Виджет может использовать
эту команду для получения данных о фактическом использовании, но это не показывает
лимиты плана напрямую.

Как только официальный эндпоинт появится (за этим следят в issue
